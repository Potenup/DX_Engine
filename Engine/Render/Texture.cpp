...
namespace Blue
{
    ...
    void Texture::LoadTexture(const std::string& name)
    {
        ...
        D3D11_SAMPLER_DESC sampleDECS = { };
        sampleDECS.Filter = D3D11_FILTER_ANISOTROPIC;
        sampleDECS.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP;
        sampleDECS.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;
        sampleDECS.AddressW = D3D11_TEXTURE_ADDRESS_CLAMP;
        sampleDECS.MaxLOD = FLT_MAX;
        sampleDECS.MinLOD = FLT_MAX;
        sampleDECS.MaxAnisotropy = 3;
        sampleDECS.ComparisonFunc = D3D11_COMPARISON_ALWAYS;

        // 샘플러 생성.
        result = device.CreateSamplerState(&sampleDECS, &textureData->samplerState);
        if (FAILED(result))
        {
